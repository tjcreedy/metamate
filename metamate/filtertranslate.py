...
class Range(argparse.Action):
    ...
    def __call__(self, parser, namespace, value, option_string=None):
        if not (self.min <= value <= self.max):
            msg = f"invalid choice: {value} (choose from [{self.min}-{self.max}])"
...
def detect_frame(seqrecords, table, pthresh=0.95, minstops=100):
    ...
    while sum(counts) < minstops or p < pthresh:
        ...
        try:
            ...
            sys.exit("Error in detect_frame: sequences exhausted without reaching minimum counts "
                     "or significance. Try reducing --detectionminstops or --detectionconfidence")
...
def getcliargs(arglist=None):
    parser = argparse.ArgumentParser(description="""
        Standalone tool for filtering the sequences in a multifasta according to whether their 
        translation contains stop codons. All sequences must have the same reading frame relative 
        to the start of the sequence. The reading frame can be supplied if known or is 
        automatically determined. All sequences must use the same translation table, which follows 
        the NCBI numbering convention (https://www.ncbi.nlm.nih.gov/Taxonomy/Utils/wprintgc.cgi)
        """)

    parser.add_argument("-i", "--input", metavar="path",
                        help="input file path", required=True)
    parser.add_argument("-t", "---table", metavar="n",
                        help="the number referring to the translation table to use",
                        type=int)
    parser.add_argument("-r", "--readingframe", metavar="n",
                        help="coding frame of sequences, if known",
                        type=int, choices=[1, 2, 3])
    parser.add_argument("-o", "--output", metavar="path",
                        help="if --outtype is 'pass', 'fail' or 'both', the output file name, or "
                             "if --outtype is 'separate', the prefix of the output file name",
                        type=str, required=True)
    parser.add_argument("-y", "--outtype", metavar="type",
                        help="one of 'pass', 'fail', 'both' or 'separate', denoting whether to "
                             "output only those that pass filtering, only those that fail "
                             "filtering, all sequences in one file adding a ';translation=pass' "
                             "or ';translation=fail' suffix in the headers, or as two separate "
                             "files with _pass or _fail suffixes to the output file name (default "
                             "pass)",
    ...
    parser.add_argument("-m", "--detectionminstops", metavar="n",
                        help="minimum number of stops to encounter for detection (default 50, may "
                             "need to decrease for few sequences)",
                        type=int, default=50)
