...
def count_asvs_in_libraries(master, librarypaths):
    ...
    if len(librarypaths) > 1:
        ...
        nameregex = r';(?:barcodelabel|sample)=([^;]+);'
        seqn = 0
        ...
        for seqr in SeqIO.parse(librarypaths[0], seqformat):
            ...
            if seq in asvseqs:
                ...
                if not libname:
                    sys.exit(f"Error: can't detect a library name in header \"{seqr.id}\" on line "
                             f"{seqn} of {librarypaths[0]}\n")
    ...
    if len(asvabsent) > 0:
        sys.exit(f"Error: {len(asvabsent)} ASV(s) were not found in the library file(s):"
                 f"\n{', '.join(asvabsent)}\n")

    # Check if all libraries had at least one match
    libabsent = [n for n in libnames if n not in countsbylibrary]
    if len(libabsent) > 0:
        sys.stderr.write(f"Warning: libraries {', '.join(libabsent)} had no sequences matching "
                         f"any ASVs\n")
...
def parse_readmap(master, mappath):
    ...
    if len(set(asvstrip)) != len(asvstrip):
        sys.stderr.write("Warning: ASV names are not unique after stripping ;size= tags, ensure "
                         "your read map file uses exactly the same ASV names as your input ASV "
                         "file\n")
    ...
    if len(sep) == 0 or len(sep) > 1:
        sys.exit("Error: cannot determine single unambiguous column separator for {mappath}, it "
                 "should be either comma or tab\n")
    else:
        sep = sep[0]
    ...
    asvcolumns = any(n in firstline for n in asvnames) or any(s in firstline for s in asvstrip)
    ...
    if asvcolumns:
        ...
        if len(firstline) != len(asvnames):
            if len(firstline) == len(asvnames) + 1:
                ...
                sys.exit(f"Error: columns of {mappath} determined to be ASVs, but number of data "
                         f"columns does not mach number of input ASVs\n")
        ...
        if all(s in firstline for s in asvstrip):
            ...
            sys.exit(f"Error: columns of {mappath} determined to be ASVs, but cannot match all "
                     f"column names with all names of input ASVs\n")
        # Parse the lines
        for n, line in enumerate(maph):
            ...
            if len(counts) != len(firstline):
                sys.exit(f"Error: number of items in line {n + 2} of {mappath} does not match "
                         f"number of columns\n")
            if not all(type(c) is int for c in counts):
                sys.exit(f"Error: not all values of line {n + 2} of {mappath} are integers")
    ...
    # Parse if ASVs are the rows
        ...
        libnames = firstline
        countsbylibrary = {ln: dict() for ln in libnames}
        readasvs = []
        for n, line in enumerate(maph):
            # n, line = list(enumerate(maph))[0]
            line = line.strip().split(sep)
    ...
    libabsent = [ln for ln, v in countsbylibrary.items() if len(v) == 0]
    if len(libabsent) > 0:
        sys.stderr.write(f"Warning: libraries {', '.join(libabsent)} had no sequences matching "
                         f"any ASVs\n")
        countsbylibrary = {ln: v for ln, v in countsbylibrary.items() if len(v) > 0}
...
def make_tree_R(scriptdir, alignpath, model, threads):
    ...
    # Run R script
    maketreecommand = subprocess.run([scriptpath,
                                      '-a', alignpath, '-m', model, '-c', str(threads)],
    ...
    if "Error" in mtcstderr:
        if 'NA/NaN/Inf in foreign function call (arg 11)' in mtcstderr:
            sys.exit("Error, UPGMA tree building failed because the aligned sequences are too "
                     "different and the distance matrix computation failed. The alignment is "
                     "probably too gappy, try making your own alignment and re-running.")
        else:
...
def get_clades_R(scriptdir, tree, height):
    ...
    getcladecommand = subprocess.run([getcladespath, '-i', str(height)],
                                     stdout=subprocess.PIPE, stderr=subprocess.PIPE, input=tree)
...
def parse_asvs(args, skipalign, skipmessage, basepath):
    ...
    if isaligned and not args.realign:
        if not skipalign:
            sys.stdout.write("Input ASVs detected as aligned (if this is not the case, run with "
                             "the --realign option). ")
        ...
        if skipalign:
            ...
            if args.realign:
                sys.stdout.write("Input ASVs detected as not aligned, but --realign ignored as "
                                 "alignment is unneeded. ")
            sys.stdout.flush()
            raw['asvs'] = SeqIO.to_dict(SeqIO.parse(args.asvs, "fasta"))
            raw['path'] = args.asvs
        else:
            sys.stdout.write("Input ASVs detected as not aligned, running MAFFT FFT-NS-1 to "
                             "align. This may take some time, skip this step by supplying an "
                             "alignment to -A/--asvs\n")
            sys.stdout.flush()
...
def find_clades(args, basepath):
    ...
    if args.tree:
        ...
        sys.stdout.write("Making a UPGMA tree from the alignment. This may take some time...\n")
        if len(aligned['asvs']) > 65536:
            exit("Error: the number of ASVs (over 65,536) is too high to compute a UPGMA tree. "
                 "Please run again with fewer ASVs. We suggest removing ASVs that are highly "
                 "infrequent and fall outside of the target length or have stops in translation. "
    ...
    # Find the clades
    sys.stdout.write(f"Finding clades from the tree at {args.divergence} divergence.\n")
